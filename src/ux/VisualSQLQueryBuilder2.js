...
var check_column = new Ext.ux.CheckColumn({
        xtype: 'checkcolumn',
		itemId: 'check_output',
		sortable: false,
        text: 'Output',
        ...
        listeners: {
            ...
            drop: function(node, data, dropRec, dropPosition){
                ...
                for (var i = 0, l = data.records.length; i < l; i++) {
                    ux.vqbuilder.sqlSelect.addFieldRecord(data.records[i], true);
                }
            }
        }
    },
...
Ext.define('Ext.ux.window.visualsqlquerybuilder.SQLTableTree', {
    ...
    initComponent: function(){
        ...
        this.store = Ext.create('Ext.data.TreeStore', {
			...
			proxy: {
				type: 'direct',
				directFn: Database.getTables,
				reader: {
					type:'json',
				}
			},
			fields: ['text', 'allowDrop', 'leaf'],
        });
    ...
    viewConfig: {
        listeners: {
            ...
            drop: function(node, data, dropRec, dropPosition){
				...
				console.log(node.dataset.boundview);
				
				
				
				//var com = Ext.getCmp(node.id);
				//console.log(com);
				
				//var com = Ext.ComponentQuery.query('#' + node.id);
               // console.log(com);
                if (node) {
					...
					sqlTable2 = Ext.getCmp(node.dataset.boundview).up('window');
                    ...
                    aBBPos = [(parseInt(data.item.dataset.recordindex) + 1), (parseInt(node.dataset.recordindex) + 1)];
					console.log(aBBPos);
